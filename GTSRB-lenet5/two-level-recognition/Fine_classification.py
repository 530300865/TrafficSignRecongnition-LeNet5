...
#图片的类别的数量, 1-6代表六个大类，不同的大类有不同的小类
def classnum(x):
    if x == "00001":
        return 8
    if x == "00002":
...
def train(train_data_dir, test_data_dir, d):
    ...
    #for i in range(901):
        ...
        #   if i % 10 == 0:
        #       saver.save(session, d+'/model.ckpt')
...
#训练数据
train_dir = os.path.join("./sorted_Training/Images")
test_dir = os.path.join("./Test_sorted/Images")

#d: 00001,00002,00003,00004,00005,00006, train_dir下面有六个文件夹，代表六个大类
tarin_directories = [d for d in os.listdir(train_dir) if os.path.isdir(os.path.join(train_dir, d))]
test_directories =  [d for d in os.listdir(test_dir) if os.path.isdir(os.path.join(test_dir, d))]

#对于每个大类，例如00001， 返回大类文件夹的路径，对其中每个小类（文件夹中的数据）进行训练，并返回模型，每一个大类都有一个模型
#若00001大类代表限速， 其下面的小类代表 限速20， 30， 40 ...， 对于每个大类的模型训练是要在限速标志中分出具体属于哪个速度的限制
for d in tarin_directories:
    train_data_dir = os.path.join(train_dir, d)#合并train_dir，d生成新的路径
    test_data_dir = os.path.join(test_dir, d)
    print()
    print("d is training ", d)
    
    train(train_data_dir, test_data_dir, d)
